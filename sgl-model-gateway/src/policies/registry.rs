...
impl PolicyRegistry {
    ...
    /// Get all policies that need periodic load updates (lock-free).
    /// This includes power_of_two and load_aware policies.
    pub fn get_all_load_polling_policies(&self) -> Vec<Arc<dyn LoadBalancingPolicy>> {
        let needs_load = |name: &str| name == "power_of_two" || name == "load_aware";

        let mut policies = Vec::new();

        if needs_load(self.default_policy.name()) {
        ...
        if let Some(policy) = prefill_policy_opt {
            if needs_load(policy.name()) && !Arc::ptr_eq(policy, &self.default_policy) {
                policies.push(Arc::clone(policy));
            }
        }

        if let Some(policy) = decode_policy_opt {
            if needs_load(policy.name())
